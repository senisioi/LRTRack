...
void ReShaper::step() {
    ...
    // do the acid!
    float out = x * (std::abs(x) + a) / (x * x + (a - 1) * std::abs(x) + 1);
...
ReShaperWidget::ReShaperWidget() {
    ...
    // ***** MAIN KNOBS ******
    addParam(createParam<LRBigKnobWhite>(Vec(35, 216), module, ReShaper::RESHAPER_AMOUNT, 1.f, 50.f, 1.f));
    addParam(createParam<LRBasicKnobWhite>(Vec(44, 120), module, ReShaper::RESHAPER_CV_AMOUNT, 0.f, 5.f, 0.f));
    // ***** MAIN KNOBS ******
